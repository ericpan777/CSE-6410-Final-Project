...
class Approx:
    ...
    def __init__(self, tsp_graph, random_seed):
        """
    ...
    def preorder(self, mst, start=10):
        ...
        random.seed(self.seed)
        start = int(round(random.uniform(1,self.tsp_graph.num_nodes),0))
        visit(start)
        return path
...
class LocalSearch:
    def __init__(self, tsp_graph, seed, steps, initial, annealing, time_limit=100):
        self.tsp_graph = tsp_graph
        self.seed = seed
        ...
        if initial not in ['ascending', 'decending', 'random', 'MST']:
            initial = 'ascending'
    ...
    def initialize(self):
        ...
        random.seed(self.seed)
        if self.initial == 'MST':
            approx_solver = Approx(tsp_graph, self.seed)
        ...
        self.k = self.distance / self.T / 5 / self.tsp_graph.num_nodes
    ...
    def annealing_function(self, step, progress):
        ...
        if self.annealing == 'linear':
            return max(self.T * (1 - progress/2), self.T_min)
        # exponentially decay
        if self.annealing == 'exponential':
            if progress == 1.:
                return 0
            else:
                return max(self.T * math.exp(self.annealing_factor * progress), self.T_min)
        # step decay
        if self.annealing == 'step':
            if step % self.M == 1:
                self.T *= self.cooling_factor
            return max(self.T, self.T_min)
        # time decay
        if self.annealing == 'time':
            ...
            if progress >= 1.:
                ...
                return max(self.T * (1 - progress), self.T_min)

    def random_swap_node(self, step):
        if step % 2 == 0:
            (idx1, idx2) = random.sample(range(1, len(self.path)), 2)
        else:
            if random.randint(0,1) == 0:
                (idx1, idx2) = random.sample(range(1, len(self.path) // 2), 2)
            else:
                (idx1, idx2) = random.sample(range(len(self.path) // 2, len(self.path)), 2)
    ...
    def solve(self):
        ...
        for i in range(self.steps):
            ...
            new_path, indices = self.random_swap_node(i)
            ...
            # update to longer distance under certain probability
                ...
                # if (pgs > 0.2 and pgs < 0.21) or (pgs > 0.4 and pgs < 0.41) or (pgs > 0.6 and pgs < 0.61) or (pgs > 0.8 and pgs < 0.81):
                #     self.k = self.best_distance / cur_T / 2000
                cur_T = self.annealing_function((i+1), ((i+1) / self.steps))
...
def output_csv(filename, cost, para):
    with open('quality.csv', 'a', newline='') as file:
        csv_writer = csv.writer(file)
        csv_writer.writerows([[filename, cost, ", ".join(para)]])
...
if __name__ == '__main__':
    ...
    if Algotype == 'BF':
        bf_solver = BF(tsp_graph, random_seed, cutoffTime-elapse)
        try:
            t1 = time.time()
            ...
            output_csv(output_filename[:-4], distance, [])
        except Exception as e:
            print("Error occurred during brute force:", e)
    elif Algotype == 'Approx':
        approx_solver = Approx(tsp_graph, random_seed)
        try:
            t1 = time.time()
            path, distance = approx_solver.solve()
            # print("Total Time:", time.time() - t1)
        ...
        ls_solver = LocalSearch(tsp_graph, random_seed, steps=int(2.5e7), initial=initial_method, annealing=annealing_method, time_limit=cutoffTime-elapse)
        try:
            t1 = time.time()
            ls_solver.solve()
            # print("Total Time:", time.time()-t1)
            ...
            output(output_filename, ls_solver.best_distance, ls_solver.best_path)
            output_csv(output_filename[:-4], ls_solver.best_distance, [initial_method, annealing_method])
            ls_solver.plot
