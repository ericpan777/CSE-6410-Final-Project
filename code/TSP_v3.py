...
class LocalSearch:
    ...
    def initialize(self):
        ...
        self.k = self.distance / self.T / 2000
...
if __name__ == '__main__':
    ...
    if Algotype == 'BF':
        ...
        initial_method = 'MST'
        annealing_method = 'time'
        ls_solver = LocalSearch(tsp_graph, random_seed, steps=int(1e7), initial=initial_method, annealing=annealing_method, time_limit=cutoffTime)
        try:
            t1 = time.time()
            ls_solver.solve()
            # print("Total Time:", time.time()-t1)
